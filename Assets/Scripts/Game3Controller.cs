...
public class Game3Controller : MonoBehaviour
{
    ...
    void SpawnEnemy()
    {
        if (Time.time >= enemyNextSpawn)
        {
            ...
            if (!spawnedBoss)
            {
                    Instantiate(EnemyLv3Prefab, new Vector3(Random.Range(-10f, -20.5f), 0.5f, Random.Range(-2.2f, 2.2f)), Quaternion.Euler(0, 90, 0));
    ...
    IEnumerator SpawnBossAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);
            Instantiate(BossLv3Prefab, new Vector3(-13f, 0.5f, 0.08f), Quaternion.Euler(0, 90, 0));
    }
