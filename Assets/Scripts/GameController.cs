...
public class GameController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        LastLevel = true;
        BossHp = 9000;
        BaseScript.LoadLevelScene = "Level2";
    }
    ...
    void SpawnEnemy()
    {
        if (Time.time >= enemyNextSpawn)
        {
            ...
            if (!spawnedBoss)
            {
                Instantiate(EnemyPrefab, new Vector3(Random.Range(-10f, -20.5f), 0.5f, Random.Range(-2.2f, 2.2f)), Quaternion.Euler(0, 90, 0));
    ...
    IEnumerator SpawnBossAfterDelay(float delay)
    {
        ...
        Instantiate(BossPrefab, new Vector3(-13f, 0.5f, 0.08f), Quaternion.Euler(0, 90, 0));
    }
